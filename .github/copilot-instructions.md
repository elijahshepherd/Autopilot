# Autopilot Copilot Instructions
...
- `extensions/`: Built-in extensions that ship with Autopilot
...
The `extensions/` directory contains first-party extensions that ship with Autopilot:
...
- If the `#runTasks/getTaskOutput` tool is available, check the `Autopilot - Build` watch task output for compilation errors. This task runs `Core - Build` and `Ext - Build` to incrementally compile Autopilot TypeScript sources and built-in extensions. Start the task if it's not already running in the background.
- If the tool is not available (e.g. in CLI environments) and you only changed code under `src/`, run `npm run typecheck-client` after making changes to type-check the main sources (it validates `./src/tsconfig.json`).
