			...
			switch (step) {
				case Step.SelectHookType: {
					...
					if (options?.target) {
						...
						pickerItems = [];
						if (shared.length > 0) {
							pickerItems.push({ type: 'separator', label: localize('hookSection.default', "Local/Autopilot AI CLI Agents") });
						...
						if (copilotOnly.length > 0) {
							pickerItems.push({ type: 'separator', label: localize('hookSection.copilotCliOnly', "Autopilot AI CLI Agents") });
