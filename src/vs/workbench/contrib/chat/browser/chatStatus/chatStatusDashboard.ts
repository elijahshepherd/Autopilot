	...
	private render(): void {
		...
		if (hasUsageSection && !this.options?.compactQuotaLayout) {
			...
			const header = this.renderHeader(headerHost, this._store, planName, toAction({
				...
				label: localize('quotaLabel', "Manage Autopilot AI Settings"),
				tooltip: localize('quotaTooltip', "Manage Autopilot AI Settings"),
	...
	private createGlobalQuotaCallout(container: HTMLElement): () => { calloutVisible: boolean; additionalUsageEnabled: boolean } {
		...
		const update = () => {
			...
			if (isEnterpriseUser && isPooledQuotaExhausted) {
				...
				calloutText.textContent = localize('quotaBudgetExceededEnterprise', "Your organization or enterprise has exceeded its Autopilot AI budget. Contact your admin to resume usage.");
			} else if (maxUsedPercentage >= 100 && additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaAdditionalUsageActiveEnterprise', "Autopilot AI has paused because your limits are reached. Please contact your admin to increase your limits.")
					: isUsageBasedBilling
				...
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaAdditionalUsageApproachingEnterprise', "Autopilot AI will pause when your limits are reached. Please contact your admin to increase your limits.")
					: isUsageBasedBilling
				...
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaPausedEnterprise', "Autopilot AI is paused until the limit resets. Contact your administrator for more information.")
					: localize('quotaPaused', "Autopilot AI is paused until the limit resets.");
			} else if (maxUsedPercentage >= 75 && !additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaWarningEnterprise', "Autopilot AI will pause when the limit is reached. Contact your administrator for more information.")
					: localize('quotaWarning', "Autopilot AI will pause when the limit is reached.");
			} else {
				quotaCallout.style.display = 'none';
			}
