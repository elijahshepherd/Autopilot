# Autopilot Downloads

Download pages for each release:

- [v0.9.0](0-9-0.html) — Collapsible reasoning, malformed-output scrubber, bounded rate-limit, capability manifest
- [v0.8.0](0-8-0.html) — Agents Window as separate admin EXE, Debug mode, AI identity, CAPI fallback
- [v0.7.0](0-7-0.html) — Auto-admin elevation, removed sign-in UI, full Autopilot rebrand, max reasoning, instant response
- [v0.6.1](0-6-1.html)
- [v0.6.0](0-6.html)
- [v0.5.0](0-5.html)
- [v0.1.0](0-1.html) — Initial release

Files are hosted on [GitHub Releases](https://github.com/elijahshepherd/Autopilot/releases).
